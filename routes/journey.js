...
router.get("/user/:userId", async (req, res) => {
  try {
    ...
    if (!journeys) {
      return res
        .status(404)
        .json({ message: "No journeys found for this user" });
    }
...
router.get("/current_user", auth, async (req, res) => {
  ...
  try {
    ...
    if (journeys.length === 0) {
      return res
        .status(404)
        .json({ error: "Không tìm thấy hành trình nào của người dùng này" });
    }
...
router.delete("/delete_all", auth, async (req, res) => {
  try {
    ...
    if (result.deletedCount === 0) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy hành trình nào để xóa" });
    }

    res
      .status(200)
      .json({ message: "Đã xóa tất cả hành trình của người dùng" });
  } catch (error) {
